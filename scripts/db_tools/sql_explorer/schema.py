...
def _is_pymssql_connection(conn: Any) -> bool:
    """Detect pymssql connection object."""
...
def _execute_with_driver_params(
    ...
    """
    Execute SQL using driver-specific placeholder style.

    - pyodbc: question-mark placeholder `?`
    - pymssql: `%s`
    """

    if _is_pymssql_connection(conn):
        sql = sql_with_qmark.replace("?", "%s")
...
def list_columns(conn: Any, schema_name: str, table_name: str) -> List[Dict[str, Any]]:
    ...
    sql = """
        ...
        """
    _execute_with_driver_params(conn, cursor, sql, (schema_name, table_name))
    return [
        {
            "name": row[0],
            "data_type": row[1],
