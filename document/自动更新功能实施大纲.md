...
- PyInstaller 现有 `excel_processor.spec` 需：
  - 将 `update/` 模块包含进主 exe（便于 `UpdateManager` 使用），同时复核 `registry/`、`page/`、`scripts/` 等现有业务模块是否已正确包含（目前 spec 未显式添加 registry，打包阶段需统一补齐）。
...
- 打包前新增检查任务：审阅 `excel_processor.spec` 与最终产物，确认 registry、更新模块、文档/模板依赖等资源均被打包，无需立即执行，待开发完成进入打包环节时执行。
