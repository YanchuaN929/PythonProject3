...
### Step3（进行中→完成）：Registry 增量写入 + finalize 管控 + 空结果缓存（负缓存）

- **你确认的业务前提**：
  - 管理员身份开始处理：允许执行 missing 归档（finalize）
  - 为性能：当源 Excel 未变化时，应尽量避免无意义 Registry 写入
  - 你额外要求：**空结果也要缓存**，避免每次都重新读Excel筛一遍

- **改动点A：空结果缓存（负缓存）**
  - `base.py::_process_with_cache()`：缓存保存条件从 `result 非空` 放宽为 `result 非 None`（允许保存空DataFrame）
  - 目的：对“长期无结果”的项目（你日志里那种读几千行最后 0 行）实现真正秒开

- **改动点B：Registry 写入改为增量触发**
  - file1~6 的每个项目循环里，新增 `should_update_registry` 判定：
    - `file_path` 在 `changed_files_for_run` 中 **或**
    - 本轮不是 cache hit（意味着真实重新解析/首次运行）
  - 只有满足上述条件，才将 raw_df 放入 `raw_results_for_registry` 并触发 `registry_hooks.on_process_done(...)`

- **改动点C：scan_finalize 管控（管理员门禁 + 有必要才执行）**
  - `update_display()` 中对 `registry_hooks.on_scan_finalize(...)` 增加条件：
    - 仅当 **管理员** 且（本轮 `changed_files_for_run` 非空 **或** 本轮确实发生过增量 Registry 写入）时执行

- **对应最小测试**：
  - `tests/test_step3_negative_cache_empty_results.py`

- **补充修复（运行期发现的边界）**：
  - 现象：当 Registry 指向“新库/路径变化/未初始化”时，`get_display_status` 可能返回空映射；原逻辑会把所有任务当作“无状态(=已忽略/已归档)”全部过滤，导致“明明处理有结果但显示全为0”。
  - 修复：`base.py::_exclude_pending_confirmation_rows()` 当 `status_map` 为空时直接降级为“跳过Registry过滤，仅保留超期过滤”。
  - 对应最小测试：`tests/test_registry_filter_fallback_when_status_map_empty.py`

### Step3 分支任务：排查“status_map 为空/状态不稳定/是否存在清空式写入”的真实根因（仅记录，不改主线设计）

背景：用户担心公共盘 Registry 会被“清空再写入”，在“缓存命中→跳过写入”时导致 tasks 变空，从而 status_map 为空；同时多用户运行会反复冲击公共盘 DB。

本仓库代码检索结论（主流程）：
- 未发现主流程存在 `DELETE FROM tasks / DROP / TRUNCATE tasks` 的“清空式写入”实现；写入路径为增量 upsert（`batch_upsert_tasks`）。
- status_map 为空更常见的真实原因：绑定到了空库/新库、或 task_id 口径不一致导致查不到。

为了在不改主程序的前提下进行自测，新增两份“只读诊断脚本”（放在 `scripts/`）：
- `scripts/check_registry_tasks_count.py`
  - 作用：输出实际连接的 `db_path`、`tasks_count`、按 `file_type` 的数量分布，用于判断“是否被清空/是否一直是空库”
- `scripts/check_registry_task_lookup.py`
  - 作用：给定界面一行的 file_type/project_id/interface_id/source_file(row basename)/row_index，计算 task_id 并查询 tasks；同时走 `hooks.get_display_status` 验证查询层是否能拿到 display_status

---

### Step4（已完成）：UI 渲染去重（处理完成仅渲染 active_tab，避免双重渲染与重复过滤日志）

**问题证据（来自你最新终端）**：
- 同一轮处理结束后，`[超期过滤]` / `[显示过滤]` / “数据加载完成/结果已显示”会出现重复块
- 根因是 `start_processing()->update_display()` 内部先调用了 `display_resultsX(...)`，随后又 `notebook.select(active_tab)` 触发 `<<NotebookTabChanged>>`，`on_tab_changed()` 再渲染一次

**本步改动点（只做渲染路径去重，不改 Registry 逻辑）**：
- `base.py::start_processing()->update_display()`：
  - 不再在 `update_display()` 内直接调用 `display_results1~6`
  - 改为仅更新：`processing_results{1..6}` / `has_processed_results{1..6}` / `update_export_button_state()`
  - 最后统一调用 `_post_processing_select_and_render_active_tab(active_tab)`：选择 active_tab 并**只渲染一次**
- `base.py::on_tab_changed()`：
  - 增加 `_suppress_tab_change_render` 抑制开关：程序内部选择 tab 时不自动触发渲染（避免重复）
  - tab1 分支统一走 `display_results(..., show_popup=False)`（避免绕过 PendingCache 覆盖）

**预期收益**：
- “开始处理”完成后，主界面只渲染当前 tab 一次，终端里重复的过滤/排序日志显著减少
- 其它 tab 的渲染改为按需：用户切换 tab 时再渲染对应结果（不影响数据正确性）

**对应最小测试（pytest）**：
- `tests/test_step4_post_processing_render_dedup.py`：验证“选择 tab + 显式渲染”总共只渲染一次
- `tests/test_step4_tab1_on_tab_changed_uses_display_results.py`：验证 tab1 切换走 `display_results`，不走 `filter_and_display_results`

#### Step4.1（补丁，已完成）：收敛 `refresh_current_tab_display()` 的“第二套渲染逻辑”

**问题证据（来自你最新终端）**：
- 在 `[指派] 用户选择暂不指派` 后仍出现其它 tab 的 `数据加载完成/处理结果已显示`，以及同一 tab 的重复渲染
- 同时出现显示口径跳变（例如同一次流程里，tab1 先显示 35 行，随后又触发另一轮渲染）

**根因**：
- `refresh_current_tab_display()` 仍保留旧的“直接 display_excel_data_with_original_rows / load_file_to_viewer”渲染路径
- 这条路径会绕过 `display_results`（PendingCache 覆盖、Registry/显示过滤等），并可能在某些 after/refresh 回调中触发，从而造成“同一轮处理出现第二次渲染/口径不一致”

**修复**：
- `base.py::refresh_current_tab_display()` 改为直接委托给 `on_tab_changed(None)`，并尊重 `_suppress_tab_change_render`，从而：
  - 只走“统一后的渲染口径”
  - 未处理状态不再触发原始预览（与 Step1 一致）
  - 避免与 Step4 的 post-processing 渲染形成二次渲染

**对应最小测试（pytest）**：
- `tests/test_step4_1_refresh_current_tab_display_delegates_to_on_tab_changed.py`

---

### Step5（高风险，暂不执行改动，仅留存方案）：ColumnSpec 单一真源的“安全落地法”

> 你指出“Step5 一旦错/漏就可能造成系统瘫痪”，我同意。这一步触及：筛选列定位、写回列定位、Registry 字段提取与 task_id 对齐，属于全栈基础设施改造，必须采用“可回退、可验证、渐进式”策略。

#### 5.1 风险点（为什么危险）
- **列定位三套表达混用**：列名 / 0-based 索引（iloc 常数）/ Excel 列字母（openpyxl）
- **file3/file6 等存在特殊分支**（列族二选一、多责任人拆分），容易遗漏
- **row_index 口径**（`原始行号` vs `idx+2`）一旦不一致，会导致 Registry task_id 不匹配、写回写错行

#### 5.2 安全纪律（强制）
- **纪律A**：任何“替换现有逻辑”的动作必须 **按 file_type 加开关**，禁止一次全量替换
- **纪律B**：每切换 1 个 file_type，必须配套最小 pytest + 你终端跑一轮确认后才能继续下一类
- **纪律C**：出现异常可一键回退到旧逻辑（开关关闭/回滚提交）

#### 5.3 渐进落地：Step5A / Step5B / Step5C
- **Step5A（0风险）：只新增 ColumnSpec + 测试，不替换任何业务代码**
  - 新增一个 ColumnSpec 模块（file1~6 的关键列：接口号/接口时间/责任人/原始行号/作废/完成/写回列等）
  - pytest 验证 ColumnSpec 覆盖 1~6 且与 `document/2_程序文件功能与逻辑说明.md` 的映射一致
- **Step5B（低风险）：只读校验模式**
  - 不改行为，只在运行时做对照校验（可开关）：硬编码列 vs ColumnSpec 指向列是否一致
  - 先形成“风险点清单”，再决定切换顺序
- **Step5C（中风险但可控）：按 file_type 开关逐类切换**
  - 一次只切 1 类文件（例如先 file1→file2→…）
  - 每类切换完成后：pytest + 终端验证通过，再进入下一类

> 结论：Step5 可以做，但必须按上述方式推进，确保“最多影响单一 file_type 且可快速回退”，避免全系统级风险。

---

## 12. Step6（待后续，放到 Step4/5 完成后再深入）：Registry“重置提示噪声/缓存已加载仍逐文件处理日志”的问题收敛

> 说明：本节只做问题收敛与后续排查计划，不做代码改动（需用户确认后再动手）。

### 12.x Step6 最新进展（已开始落地）：先做“去噪 + 更严格的时间标准化比较”

你提供的终端里出现了两类典型“噪声”：
- 批量 upsert 时大量逐条：`[Registry] 接口XXX: 预期时间变化，重置状态`
- 忽略快照检查逐条打印（快照/当前/行号等），在多任务场景下会刷屏

本轮 Step6 我优先采用“**不改业务含义，只减少误判与日志刷屏**”的最小改动策略：

#### 12.x.1 更严格的“预期时间变化”比较（减少格式差异导致的假变化）
- 位置：`registry/service.py::should_reset_task_status`
- 行为：
  - 支持 `yyyy-mm-dd / yyyy.mm.dd / yyyy年m月d日`
  - 支持 `mm.dd / mm-dd / mm/dd`：若另一侧包含年份，则补齐年份后比较
  - 避免把 `25C2` 这类非日期字符串误当成日期

#### 12.x.2 Registry 日志去噪（默认汇总，按需逐条）
- 位置：`registry/service.py::batch_upsert_tasks`
- 行为：
  - 默认不再逐条打印每个接口的“预期时间变化→重置状态”
  - 改为批量结束后输出一条汇总：重置了多少条（附带最多3条示例接口号）
  - 若需要逐条排查：设置环境变量 `REGISTRY_VERBOSE=1` 可恢复详细打印
  - 忽略快照检查的详细打印同样受 `REGISTRY_VERBOSE` 控制；默认只在真正“自动取消忽略”时输出关键行

#### 12.x.3 对应最小测试（pytest）
- `tests/test_step6_should_reset_task_status_normalization.py`
  - 覆盖：`12.03` vs `2025-12-03` 不应触发重置；真实日期变化应触发；`25C2` 不应被当作日期

### 12.y Step6 新子领域（需要围绕 Registry 深入排查）：为什么“新旧时间”在多次运行中仍无法稳定对齐？

#### 12.y.1 现象（你最新终端中的关键矛盾）
- 你只对 **文件2（项目1907）的一条接口** 写入了“责任人”（其他列未改）。
- 但在随后的 `batch_upsert_tasks` 中，出现了汇总：`预期时间变化→重置状态 9 条`（示例接口号见终端）。

> 结论：问题不是“日志逐条/汇总”的表现形式，而是：Registry 在比较 old_time vs new_time 时，为什么会把 9 条判定为“变化”？

#### 12.y.2 高概率根因假设（按可能性排序）

1) **old_time 为空/无效，new_time 有值 → 被当作变化**
   - `should_reset_task_status` 的逻辑只要 `old_time_norm != new_time_norm` 就会重置；
   - 如果 old_time_norm 为 `""`（历史写入为空/解析失败/字段缺失），而 new_time_norm 为 `2025-12-03`，会被当作“变化”。
   - 这能解释“只改责任人仍触发多条重置”：因为文件变化导致对该项目 **全量 upsert（116条）**，其中一部分历史记录的 `interface_time` 可能一直是空或脏值。

2) **同一接口的历史记录 old_time 存在，但格式/解析口径不稳定导致比较不等**
   - 例如旧：`2025.12.03` / `2025年12月3日` / 带空格；新：`2025-12-03`；
   - 或旧：`12.03`（无年份），新：`2025-12-03`（带年份）。
   - 这种属于“格式差异/标准化策略差异”导致的“假变化”（需要靠一致的标准化规则来消除）。

3) **old_task 选取的版本不是你预期的那条（business_id 对应多版本/多来源）**
   - Registry 通过 `business_id = file_type|project_id|interface_id` 继承；
   - 如果历史上存在多条同 business_id 的记录，`find_task_by_business_id` 可能取到“最后 seen”的那条，而那条的 `interface_time` 恰好为空/旧格式，从而触发变化。

4) **写入链路中，某些情况下 fields['interface_time'] 根本没被写入（导致长期为空）**
   - 这会导致 old_time 经常是空，从而在下一次“终于写到正确时间”时触发重置。
   - 需要核对：`registry/util.py::build_task_fields_from_row` 对 file2 的时间列提取是否稳定；以及处理侧 DataFrame 的 `接口时间` 是否总是非空。

5) **绑定数据库路径/数据源切换导致“旧数据不存在”**
   - 如果某次运行绑定到空库/新库，旧数据缺失；下一次又绑定回原库，会出现“看起来继承/比较失真”的现象。
   - 你终端中会打印绑定路径：`[Registry] 本轮处理绑定数据库: ...registry.db`，需确认每次一致。

#### 12.y.3 关键业务口径确认（决定 Step6 的最终修复策略）

为确保不引入新的状态误判，需要你确认一句话（建议口径）：
- **规则R**：只有当“旧时间”和“新时间”都能被识别为有效日期，且日期确实不同，才允许触发“重置状态”；若旧时间为空/无效或新时间为空/无效，应视为“数据补全/解析波动”，**不重置**。

#### 12.y.4 建议的取证/排查动作（不改主流程，也能定位根因）

1) **对 9 条被重置的接口抽样取证（1~3条即可）**
   - 目的：确认 old_time 是否为空/无效，或只是格式不同。
   - 方法：用现有脚本 `scripts/check_registry_task_lookup.py` 查询对应接口（使用终端汇总里的示例接口号）。

2) **对照 Excel 源表**
   - 同样抽 1~3 条接口，在 `内部接口信息单报表1907...xlsx` 中检查“答复期限/接口时间”单元格是否本来就有值。

3) **核对 Registry 写入字段来源链路（处理DF → registry/util → service）**
   - 重点：file2 的 `interface_time` 在 `result_df` 中列名/列序号是否稳定，是否存在某次写入缺失。

> 说明：这一子领域属于“Registry 数据一致性/历史继承”的核心问题，确实是一个新的检查方向；完成取证后再决定是“规则R落地修复”还是“修正旧数据来源/旧任务选取策略”。

### 12.1 现象A：已提示“成功加载缓存”，但日志仍出现逐项目“处理项目X的文件Y/处理文件6: …”的输出

**证据（来自你最新终端）**：
- `✅ 成功加载 35 个缓存结果`（刷新阶段）
- 随后仍出现类似：
  - `开始批量处理文件2类型... / 处理项目1907的文件2...`
  - `开始批量处理文件3类型... / 处理项目1818的文件3...`
  - `处理文件6: 收发文清单1818.xlsx ... 2306.xlsx`

**初步判断（不等同于“真的重读Excel”）**：
- 当前实现是“**外层仍进入批处理循环**”，并在循环内对每个项目打印“处理项目…”；
- 但在 Step2 快路径下，若 `file_path` 未变化且 refresh 阶段已有 raw 结果，会复用内存缓存，理论上不会重新解析 Excel；
- 因此此现象更像是“日志粒度偏粗/外层循环日志不可避免”，并不必然代表冗余 IO。

**后续（等 Step4/5 后）可做的优化方向**：
- 将“缓存复用命中”的分支单独打印为“cache-hit/skip-processing”，避免误解为重复处理；
- 或在 Step4（UI渲染去重）一起收敛日志输出，减少重复的“处理完成/显示过滤/超期过滤”块。

### 12.2 现象B：仅对 1907 的 file2 指派 1 个接口，却出现大量 “interface_time 预期时间变化 → 重置状态” 的提示

**证据（来自你最新终端）**：
- 指派后文件保存：`[指派] 文件已保存: ...内部接口信息单报表190720251203.xlsx`
- 随后检测到 `1 个文件变化`，触发对 **文件2项目1907** 的重处理与 `on_process_done`：
  - `[INFO] Registry: 文件2项目1907写入116个任务`
- 在批量 upsert 期间出现大量提示：
  - `接口XXX: 预期时间变化，重置状态`
  - 同时伴随：`行号变化较大(旧行=..., 新行=...)，但仍继承状态`

**关键澄清（业务认知对齐）**：
- “指派 1 条”会触发 **单条任务的即时更新**（你日志中也出现“共更新1个任务”）；
- 但因为源文件被写入导致“文件变化”，本轮会对 **文件2项目1907 的全部筛选结果** 做一次 `on_process_done` 批量 upsert（这是 Step3 的“增量”预期行为：按文件粒度，而非按单行粒度）。

**为什么会出现“大量预期时间变化”提示（候选根因，待验证）**：
- 预期时间字段的**格式标准化口径变化**导致字符串不相等（如 `2025.12.03` vs `2025-12-03`）；
- 预期时间字段的**解析来源列/解析策略变化**（例如某些行本轮解析失败变空/或从空变有）；
- task 匹配路径发生变化（business_id/row_index 继承命中到不同历史记录）导致比较的“旧时间”不是预期那条；
- 同一 business_id 存在多版本任务，匹配到的旧记录不同，引发大量“变化判定”。

**后续排查计划（明确放到 Step4/5 之后）**：
- Step5（ColumnSpec）完成后，先把 “file2 的 interface_time 字段来源列/解析/标准化” 固定为单一真源；
- 再用 `scripts/check_registry_task_lookup.py` 选取 1~3 条被重置的接口做“旧值 vs 本轮新值”的精确对比，确认到底是“真实变化”还是“格式/解析口径变化”；
- 若确认是“格式差异导致假变化”，再考虑在 registry/service 的比较逻辑中做更严格的时间标准化或等价比较（需用户确认后再改）。
