...
- 在不破坏现有 find→process→export 流程与界面逻辑的前提下，新增“中心登记簿 + 记录追踪”，解决：
  - 设计人员回填后，上级仍可见并可确认；
- 所有角色（含设计人员）均可按需隐藏长期过期项，默认为上级角色开启；设计人员也提供同样的过滤开关（可配置默认值）（第一步仅记录，第二步再接UI）。
...
- 视图过滤相关（第二步UI使用）：
  - `view_hide_overdue_for_designer_default`（bool，默认 True，设计人员是否默认隐藏长期过期项）
  - `view_overdue_days_threshold`（int，默认30，用于“长期过期”的界面过滤阈值；不影响现有标红/导出逻辑）
...
- 设计人员视图：支持一键隐藏“长期过期”干扰项的过滤开关（默认开启，可关闭）；该过滤仅作用于界面展示，不改变底层筛选/导出与任务状态。
