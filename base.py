...
class ExcelProcessorApp:
    ...
    def _sync_registry_source_results(
        self,
        *,
        file_type,
        entries,
        ...
        registry_scan_scopes,
    ):
        """同步一类文件的原始结果，并仅记录完整成功的扫描范围。"""
        if registry_hooks is None:
            return

        changed_files = set(changed_files or set())
        for source_file, project_id, raw_df, cache_hit in entries or []:
            try:
                if raw_df is None:
                    continue
                ...
                if not raw_df.empty:
                    use_fast_touch = bool(
                        cache_hit
                        and not registry_bootstrap_needed
                        and source_file not in changed_files
                        and hasattr(registry_hooks, "on_cached_process_done")
                    )
                    hook = (
                        registry_hooks.on_cached_process_done
                        if use_fast_touch
                        else registry_hooks.on_process_done
                    )
                    success = hook(
                        file_type=file_type,
                    ...
                    if success is not False:
                        registry_write_flags["count"] = registry_write_flags.get("count", 0) + 1

                if success is not False:
                    registry_scan_scopes.append({
                        "file_type": int(file_type),
                        "project_id": str(project_id),
                        "source_file": source_file,
                    })
            except Exception as exc:
                print(f"[Registry] 文件{file_type}项目{project_id}同步失败: {exc}")
        ...
        for file_type in (5, 6, 7):
            ...
            for file_path, project_id in targets:
                try:
                    ...
                    raw_results_for_registry.append(
                        (file_path, project_id, raw_result, cache_hit)
                    )
            ...
            self._sync_registry_source_results(
                file_type=file_type,
                entries=raw_results_for_registry,
                registry_bootstrap_needed=registry_bootstrap_needed,
                changed_files=changed_files,
                registry_write_flags=registry_write_flags,
                registry_scan_scopes=registry_scan_scopes,
            )
            ...
            try:
                ...
                if process_file1 and self.target_files1:
                    if hasattr(main, 'process_target_file'):
                        ...
                        new_multi1 = {}
                        combined_results = []
                        # 保存角色筛选前结果，缓存命中时也要刷新 Registry 存活状态。
                        raw_results_for_registry = []
                ...
                try:
                    ...
                    should_finalize = bool(is_admin and registry_scan_scopes)
                    if should_finalize:
                        registry_hooks.on_scan_finalize(
                            batch_tag=batch_tag,
                            scanned_sources=registry_scan_scopes,
                        )
                    folder_path = self.config.get('folder_path', '').strip()
                    if folder_path:
                        from registry.config import load_config
