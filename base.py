...
try:
    ...
    def get_file1_db_connection_status():
        return {"connected": "0", "message": "未连接：数据库驱动未就绪", "connector": ""}
...
class ExcelProcessorApp:
    ...
    def __init__(self, auto_mode: bool = False, resume_action: str = ""):
        ...
        self.file1_db_status = {"connected": "0", "message": "未检测", "connector": ""}
    ...
    def _format_file1_db_status(status: Dict[str, str]) -> str:
        ...
        if not message:
            message = "未连接：未知错误"
        return f"✓ 待处理文件1 (内部需打开接口/数据库): {message}"
    ...
    def load_config(self):
        ...
        if "file1_use_db_mode" not in self.config:
            self.config["file1_use_db_mode"] = True
    ...
    def _apply_pending_overrides(self, df, file_type):
        try:
            if file_type == 1 and self._is_file1_db_mode_enabled():
                return df
    ...
    def refresh_file_list(self, show_popup=True):
        ...
        def refresh_worker():
            ...
            db_path = ""
            db_error = ""
            file1_db_status = {"connected": "0", "message": "未连接：未检测", "connector": ""}
            try:
                ...
                if self._is_file1_db_mode_enabled():
                    file1_db_status = get_file1_db_connection_status()
                ...
                if self.excel_files:
                    ...
                    if self.target_files1:
                        if self._is_file1_db_mode_enabled():
                            file_info += f"✓ 待处理文件1 (数据库任务): {len(self.target_files1)} 个项目\n"
                            for _, pid in self.target_files1:
                                disp_pid = pid if pid else "未知项目"
                                file_info += f"  - {disp_pid}\n"
                    ...
                    if self.target_file1:
                        if self._is_file1_db_mode_enabled():
                            file_info += f"  内部需打开接口: 数据库任务 (项目{self.target_file1_project_id})\n"
                        else:
            ...
            try:
                ...
                if process_file1 and self.target_files1:
                    if hasattr(main, 'process_target_file'):
                        ...
                        file1_db_mode = self._is_file1_db_mode_enabled()
                        for file_path, project_id in self.target_files1:
                            try:
                                ...
                                # Step2：优先复用 refresh 阶段已加载到内存的 raw 缓存（仅Excel模式）
                                result = None
                                used_refresh_cache = False
                                if (not file1_db_mode) and can_reuse_refresh_cache and (file_path not in changed_files_for_run):
                                ...
                                if result is None:
                                    if file1_db_mode:
                                        ...
                                        # Excel模式：使用缓存处理（.pkl）或缓存未命中则处理Excel
                                ...
                                if (not file1_db_mode) and (not cache_hit):
                                    try:
    ...
    def display_results(self, results, show_popup=True):
        ...
        display_df = self._ensure_source_file_column_for_pending_cache(results, "内部需打开接口")
        display_df = self._apply_pending_overrides(display_df, 1)
    ...
    def display_results2(self, results, show_popup=True):
        ...
        display_df = self._ensure_source_file_column_for_pending_cache(results, "内部需回复接口")
        display_df = self._apply_pending_overrides(display_df, 2)
        # 不要drop原始行号列，因为需要它来加载勾选状态
    ...
    def display_results3(self, results, show_popup=True):
        ...
        display_df = self._ensure_source_file_column_for_pending_cache(results, "外部需打开接口")
        display_df = self._apply_pending_overrides(display_df, 3)
        # 不要drop原始行号列，因为需要它来加载勾选状态
        excel_row_numbers = list(display_df['原始行号'])
    ...
    def display_results4(self, results, show_popup=True):
        ...
        display_df = self._ensure_source_file_column_for_pending_cache(results, "外部需回复接口")
        display_df = self._apply_pending_overrides(display_df, 4)
        # 不要drop原始行号列，因为需要它来加载勾选状态
        excel_row_numbers = list(display_df['原始行号'])
    ...
    def display_results5(self, results, show_popup=True):
        ...
        self.has_processed_results5 = True
        display_df = self._ensure_source_file_column_for_pending_cache(results, "三维提资接口")
        display_df = self._apply_pending_overrides(display_df, 5)
        # 不要drop原始行号列，因为需要它来加载勾选状态
        excel_row_numbers = list(display_df['原始行号'])
    ...
    def display_results6(self, results, show_popup=True):
        ...
        self.has_processed_results6 = True
        display_df = self._ensure_source_file_column_for_pending_cache(results, "收发文函")
        display_df = self._apply_pending_overrides(display_df, 6)
        # 不要drop原始行号列，因为需要它来加载勾选状态
        excel_row_numbers = list(display_df['原始行号'])
                ...
                if isinstance(results, pd.DataFrame) and not results.empty:
                    ...
                    for file_path, pid in self.target_files1:
                        if pid == project_id:
                            ...
                            break
                    # 文件1数据库模式只读：不走完成勾选/Registry过滤
                    if original_file and not file1_db_mode:
                    ...
                    if file1_db_mode:
                        results = build_file1_export_dataframe(results)
                    # 保存过滤后的结果
    ...
    def _check_unassigned_tasks(self):
        ...
        try:
            ...
            processed_results = {}
            for i in range(1, 7):
                if i == 1 and self._is_file1_db_mode_enabled():
                    # 文件1数据库只读，不参与指派
                    continue
